<?php
  ...
  $nome = $_GET['nome'];
  echo $nome;
  $query="SELECT * FROM CULTURA WHERE NOME='".$nome."'";
  $queryfoto = "SELECT * FROM FOTO WHERE NOME LIKE '%".$nome."%'";
  $querypercorso ="SELECT percorso FROM PERCORSOCULTURA WHERE cultura='".$nome."'";
  ...
  if ($result = $connection->query($query)) {
    if ($resultfoto = $connection->query($queryfoto)){
        if($resultpercorso = $connection->query($querypercorso)){
            $riga = $resultpercorso->fetch_array(MYSQLI_ASSOC);
            echo $riga;
